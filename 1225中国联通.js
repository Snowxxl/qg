...
- 运行逻辑：抢券任务仅在 09:58 - 10:10 时间段内触发，其余时间自动跳过。
- 策略建议：请务必添加 9:58 的定时任务以保证命中抢购窗口。
...
/**
 * Got v13+ compatibility wrapper for this script.
 * - Provides .extend(), .get(), .post() and .defaults.options.timeout.request
 * - Internally calls the ESM default export of got
 */
function createGot(baseOptions = {}) {
  const timeoutOpt = baseOptions.timeout;
  const normalizedTimeout =
    typeof timeoutOpt === "object"
      ? timeoutOpt
      : { request: timeoutOpt || requestTimeout };

  const base = { ...baseOptions, timeout: normalizedTimeout };

  const client = (opts = {}) => got({ ...base, ...opts });
  client.get = (url, opts = {}) => got({ ...base, ...opts, url, method: "get" });
  client.post = (url, opts = {}) => got({ ...base, ...opts, url, method: "post" });
  client.extend = (opts = {}) => createGot({ ...base, ...opts });
  client.defaults = { options: base };
  return client;
}
...
class UserService {
  constructor() {
    ...
    // 修改点：用 createGot() 代替 got.extend()
    this.got = createGot(httpClientOptions);
...
// ============================================
  ...
  // 4. 任务主入口
  async grab50_task() {
    // 这里加个时间判断，避免全天运行浪费资源，只在 9:58 - 10:05 之间运行
    const now = new Date();
    const hour = now.getHours();
    const minutes = now.getMinutes();
    
    // 如果不是在 9点58分 到 10点10分之间，直接跳过 (你可以根据需要修改这个限制)
    if (!(hour === 9 && minutes >= 58) && !(hour === 10 && minutes <= 10)) {
        this.log("抢50话费: 当前不在活动时间(09:58-10:10)，跳过");
        return;
    }
...
// === 修改为串行模式：登录+任务 一气呵成 ===
  // 这样保证轮到该账号时，才获取IP，并在IP有效期内立刻跑完任务
  for (let user of appName.userList) {
    // 1. 获取IP并登录
    // (userLoginTask 里我们已经加了 set_proxy_ip，所以这里一运行就会去拿新IP)
    await user.userLoginTask();
    
    // 2. 如果登录成功，立刻执行该账号的所有任务
    if (user.valid) {
      await user.userTask();
    }
    
    // (可选) 账号间休息几秒，防止并发过快（虽然是串行，但缓冲一下更稳）
    await appName.wait(2000);
  }
